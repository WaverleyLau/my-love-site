#!/usr/bin/env node
const { CodeReviewer } = require('./review-script.cjs');
const path = require('path');

/**
 * 代码审查执行脚本
 * 用法: node .trae/skills/code-review/run-review.cjs [文件路径...]
 */
...
async function main() {
  ...
  if (filePaths.length === 0) {
    console.log('用法: node .trae/skills/code-review/run-review.cjs <文件路径...>');
    console.log('示例: node .trae/skills/code-review/run-review.cjs src/App.vue src/main.ts');
    process.exit(1);
  }
